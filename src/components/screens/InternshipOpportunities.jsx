  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs text-slate-500 font-medium mb-1">
            ...
            <span>›</span>
            <span className="text-slate-900 font-bold">Opportunities Hub</span>
            <span>›</span>
            <span className="text-blue-600 font-bold">{selectedTab}</span>
          </div>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-bold transition self-start sm:self-auto cursor-pointer"
        >
      ...
      <div className="bg-white rounded-2xl p-3 sm:p-4 border border-slate-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-800 placeholder-slate-400"
          />
        </div>

        <label className="flex items-center gap-2 cursor-pointer select-none text-xs font-semibold text-slate-700 self-start sm:self-auto px-1">
          <input
            type="checkbox"
            checked={remoteOnly}
      ...
      <div className="space-y-4">
        {filtered.map((opp) => {
          ...
          return (
            <div
              key={opp.id}
              className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs hover:border-blue-300 hover:shadow-md transition space-y-4"
            >
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                <div className="flex items-start gap-3.5">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-slate-500">
                      ...
                      <span>•</span>
                      <span className="text-[11px] text-slate-400">Source: <strong className="text-slate-600">{opp.source || "LinkedIn"}</strong></span>
                      <span>•</span>
                      <span className="text-[11px] text-rose-600 font-semibold">Deadline: {opp.deadline || "Open"}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="sm:text-right shrink-0">
                  <span className="inline-block px-3 py-1 bg-emerald-50 text-emerald-700 text-xs font-black rounded-lg border border-emerald-200">
              ...
              <div className="p-3 bg-slate-50/80 rounded-xl border border-slate-200/70 text-xs space-y-2">
                <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
                  <span className="font-bold text-slate-600">Why you match:</span>
                  {opp.matchingSkills?.slice(0, 3).map((m, idx) => (
                    <span
                      key={idx}
                      className="px-2 py-0.5 bg-emerald-50 text-emerald-700 font-semibold rounded border border-emerald-200"
                    >
                      ✓ {m.skill} ({m.studentScore}%)
                    </span>
                  ))}
                  {opp.missingSkills?.length > 0 && (
                    <span className="px-2 py-0.5 bg-rose-50 text-rose-700 font-semibold rounded border border-rose-200">
                      ⚠ {opp.missingSkills[0].skill} gap (-{opp.missingSkills[0].gap}%)
                    </span>
                  )}
                </div>

                <div className="flex items-center justify-between text-[11.5px] text-slate-500 pt-0.5">
                  <span className="line-clamp-1">{opp.recommendation}</span>
                  <button
                    onClick={() => setActiveModalOpp(opp)}
                    className="text-blue-600 font-bold hover:underline shrink-0 ml-2 cursor-pointer flex items-center gap-0.5"
                  >
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-xs">
                <button
                  onClick={() => setActiveModalOpp(opp)}
                  className="font-semibold text-slate-600 hover:text-blue-600 transition cursor-pointer"
                >
        ...
        {filtered.length === 0 && (
          <div className="bg-white rounded-2xl p-10 text-center border border-slate-200/80 shadow-xs space-y-3">
            ...
            <h3 className="text-sm font-bold text-slate-800">No opportunities found</h3>
            ...
            <button
              ...
              className="px-4 py-2 bg-blue-50 text-blue-700 text-xs font-bold rounded-xl hover:bg-blue-100 transition cursor-pointer"
            >
              Reset Filters
            </button>
          </div>
      ...
      {activeModalOpp && (
        ...
        >
          <div
            className="bg-white rounded-2xl max-w-xl w-full p-6 space-y-5 border border-slate-200 shadow-2xl relative overflow-y-auto max-h-[90vh]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between pb-3 border-b border-slate-100">
              <div>
                <span className="text-[10px] font-bold px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full uppercase tracking-wider">
                  SkillBridge Competency Analysis
                </span>
                <h3 className="text-base font-black text-slate-900 mt-1">
                  Why you match: {activeModalOpp.role}
                </h3>
                <p className="text-xs text-slate-500">{activeModalOpp.company} • {activeModalOpp.location}</p>
              </div>
              <button
                onClick={() => setActiveModalOpp(null)}
            ...
            <div className="p-4 rounded-xl bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 flex items-center justify-between">
              <div>
                <span className="text-[11px] font-semibold text-slate-600 block">Current Profile Match</span>
                <span className="text-2xl font-black text-blue-700">{activeModalOpp.matchPercentage}%</span>
              </div>
              <div className="text-right">
                <span className="text-[11px] font-semibold text-slate-600 block">Potential After Roadmap</span>
                <span className="text-2xl font-black text-emerald-600">{activeModalOpp.potentialMatchPercentage}%</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {activeModalOpp.matchingSkills?.map((m, idx) => (
                  <div key={idx} className="p-2.5 rounded-lg bg-emerald-50/50 border border-emerald-100 flex items-center justify-between text-xs">
                    <span className="font-semibold text-slate-800">{m.skill}</span>
                    <div className="flex items-center gap-3">
                      <span className="text-slate-500">Your Score: <strong className="text-slate-800">{m.studentScore}%</strong></span>
                      <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 font-bold rounded text-[10.5px]">✓ {m.status}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  {activeModalOpp.missingSkills.map((gap, idx) => (
                    <div key={idx} className="p-2.5 rounded-lg bg-rose-50/50 border border-rose-100 flex items-center justify-between text-xs">
                      <div>
                        <span className="font-semibold text-slate-900 block">{gap.skill}</span>
                        <span className="text-[11px] text-slate-500">Impact: <strong className="text-emerald-700">{gap.impact}</strong></span>
                      </div>
                      <span className="px-2 py-0.5 bg-rose-100 text-rose-800 font-bold rounded text-[10.5px]">{gap.gap}% Gap</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          >
            ...
            <div className="flex items-start justify-between pb-3 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500">{applyModalOpp.company} • {applyModalOpp.stipend}</p>
              </div>
              <button
            ...
            {applyStep < 5 && (
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-[11px] font-bold text-slate-600">
                  <span className={applyStep >= 1 ? "text-blue-600" : "text-slate-400"}>1. Profile</span>
                  <span>›</span>
                  <span className={applyStep >= 2 ? "text-blue-600" : "text-slate-400"}>2. Resume</span>
                  <span>›</span>
                  <span className={applyStep >= 3 ? "text-blue-600" : "text-slate-400"}>3. Skills</span>
                  <span>›</span>
                  <span className={applyStep >= 4 ? "text-blue-600" : "text-slate-400"}>4. Review</span>
                </div>
            ...
            {applyStep === 1 && (
              <div className="space-y-4 text-xs">
                <div className="p-3 bg-blue-50/60 rounded-xl border border-blue-100 text-blue-950 leading-relaxed">
            ...
            {applyStep === 2 && (
              <div className="space-y-4 text-xs">
                <div className="space-y-2">
                  <label className="block text-[11px] font-bold text-slate-700">Resume Submission Method</label>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <span className="font-bold text-slate-900 block">Verified Skill Passport</span>
                      <span className="text-[10.5px] text-slate-500">Auto-generates verified portfolio PDF</span>
                    </div>
                    ...
                    >
                      <span className="font-bold text-slate-900 block">Custom Resume Upload</span>
                      <span className="text-[10.5px] text-slate-500">Upload PDF / DOCX from device</span>
                    </div>
                  </div>
                </div>

                {resumeType === "upload" && (
                  <div className="p-4 border-2 border-dashed border-slate-200 rounded-xl text-center space-y-1 bg-slate-50/50">
                    <Upload className="w-6 h-6 text-slate-400 mx-auto" />
                    <span className="font-semibold text-slate-700 block">Sachin_Resume_DataAnalyst.pdf (245 KB)</span>
                    <span className="text-[10.5px] text-emerald-600 font-bold">✓ Uploaded and parsed</span>
                  </div>
                )}

                <div>
                  <label className="block text-[11px] font-bold text-slate-700 mb-1">GitHub / Live Project Portfolio</label>
                  <input
                    type="url"
                    value={portfolioLink}
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-700 mb-1">Short Candidate Note (Optional)</label>
                  <textarea
                    rows={2}
                    value={coverNote}
                    onChange={(e) => setCoverNote(e.target.value)}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-800 text-xs"
                  />
                </div>
            ...
            {applyStep === 3 && (
              <div className="space-y-4 text-xs">
                <div className="p-3 bg-purple-50 rounded-xl border border-purple-200 flex items-start gap-2 text-purple-900">
                ...
                <label className="flex items-center gap-2 p-3 bg-slate-50 rounded-xl border border-slate-200 cursor-pointer">
                  ...
                  />
                  <div>
                    <span className="font-bold text-slate-900 block">Attach SkillBridge Passport (ID: SB-2026-88492-V)</span>
                    <span className="text-[10.5px] text-slate-500">Includes 12 verified skill badges, assessment scores, and project repo links</span>
                  </div>
                </label>

                <div className="space-y-1.5">
                  <span className="font-bold text-slate-700 block">Matching Competencies Being Shared:</span>
                  <div className="flex flex-wrap gap-1.5">
                    {applyModalOpp.matchingSkills?.map((m, idx) => (
                      <span key={idx} className="px-2.5 py-1 bg-emerald-50 text-emerald-800 rounded-lg border border-emerald-200 font-bold text-[11px]">
                        ✓ {m.skill} ({m.studentScore}%)
                      </span>
                    ))}
                  </div>
              ...
              <div className="space-y-4 text-xs">
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-2.5">
                  <div className="flex justify-between pb-2 border-b border-slate-200/80">
                    ...
                    <span className="font-bold text-slate-900">{applicantName} ({applicantEmail})</span>
                  </div>
                  <div className="flex justify-between pb-2 border-b border-slate-200/80">
                    <span className="text-slate-500">Applying For:</span>
                    <span className="font-bold text-slate-900">{applyModalOpp.role} at {applyModalOpp.company}</span>
                  </div>
                  <div className="flex justify-between pb-2 border-b border-slate-200/80">
                    <span className="text-slate-500">Match Affinity:</span>
                    <span className="font-black text-emerald-600">{applyModalOpp.matchPercentage}% Verified Match</span>
                  </div>
                  <div className="flex justify-between pb-2 border-b border-slate-200/80">
                    <span className="text-slate-500">Credential Shared:</span>
                    <span className="font-bold text-purple-700">Digital Skill Passport (Verified)</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Portfolio:</span>
                    <span className="font-mono text-blue-600 truncate max-w-[200px]">{portfolioLink}</span>
                  </div>
                </div>
            ...
            {applyStep === 5 && submittedAppDetails && (
              <div className="text-center py-4 space-y-4 animate-in zoom-in-95 duration-200">
                ...
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 max-w-sm mx-auto text-xs text-left space-y-1">
                  <div className="flex justify-between">
                    <span className="text-slate-500">Tracking ID:</span>
                    <span className="font-mono font-bold text-slate-800">{submittedAppDetails.id}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Current Stage:</span>
                    <span className="font-bold text-blue-600">Applied (Under Review)</span>
                  </div>
                </div>
                ...
                <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-3">
                  <button
                    onClick={() => setApplyModalOpp(null)}
                    className="w-full sm:w-auto px-4 py-2 border border-slate-200 hover:bg-slate-50 text-slate-700 text-xs font-bold rounded-xl transition cursor-pointer"
                  >
