  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-6 border border-slate-200/80 shadow-xs space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div>
                <h3 className="text-sm font-bold text-slate-800">Recent Applications Received</h3>
                <p className="text-[11px] text-slate-500">Applications submitted with verified Digital Skill Passports</p>
              </div>
              <button
                onClick={() => setCurrentTab("candidate_matching")}
                  ...
                  {applications.map((app, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/70 transition">
                      <td className="py-3 font-semibold text-slate-800 flex items-center gap-2.5">
                        ...
                        <div>
                          <span>{app.name}</span>
                          <span className="text-[10px] text-slate-400 block">{app.skills.join(", ")}</span>
                        </div>
                      </td>
      ...
      {currentTab === "post_jobs" && (
        <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs space-y-5">
          <div className="pb-3 border-b border-slate-100">
            <h3 className="text-base font-bold text-slate-900">Post a New Opportunity</h3>
          ...
          <form onSubmit={handlePostJobSubmit} className="space-y-4 text-xs">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-700 mb-1">Opportunity Type</label>
                <select
                  value={jobCategory}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-[11px] font-bold text-slate-700 mb-1">Stipend / Annual Compensation</label>
                <input
                  type="text"
                  required
                  value={stipend}
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-700 mb-1">Location & Mode</label>
                <input
                  type="text"
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-700 mb-1">Required Competencies (Comma-separated)</label>
              <input
                type="text"
                required
      ...
      {currentTab === "candidate_matching" && (
        <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs space-y-4">
          <div className="pb-3 border-b border-slate-100 flex items-center justify-between">
            <div>
              <h3 className="text-base font-bold text-slate-900">Pre-Verified Candidate Pool</h3>
              <p className="text-xs text-slate-500">Sorted by verified competency graph alignment</p>
            </div>
          ...
          <div className="space-y-3">
            {applications.map((cand, idx) => (
              <div
                key={idx}
                className="p-4 rounded-xl border border-slate-200/80 hover:border-blue-300 transition flex flex-col sm:flex-row sm:items-center justify-between gap-3"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">{cand.role} • Apex University</p>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {cand.skills.map((s, sIdx) => (
                        <span key={sIdx} className="px-1.5 py-0.2 bg-slate-100 text-slate-600 text-[10px] rounded">
                          ✓ {s}
                        </span>
                      ))}
                    </div>
      ...
      {currentTab === "feedback" && (
        <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs space-y-4">
          <div className="pb-3 border-b border-slate-100">
            <h3 className="text-base font-bold text-slate-900">Post-Interview Candidate Feedback</h3>
          ...
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3 text-xs">
            <div>
              <label className="block text-[11px] font-bold text-slate-700 mb-1">Select Candidate</label>
              <select className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl font-medium text-slate-800">
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-700 mb-1">Interview Assessment Note</label>
              <textarea
                rows={3}
